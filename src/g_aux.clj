;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; The Dactyl-ManuForm Keyboard ù Opposable Thumb Edition              ;;
;; Auxiliary Features                                                  ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(ns dactyl-keyboard.cad.aux
  (:require [scad-clj.model :exclude [use import] :refer :all]
            [scad-tarmi.core :refer [p]]
...
(defn mcu-position
  ...
  (let [corner (getopt :mcu :position :corner)
   ...
   (->>
     ...
     ;; Face the cornerÆs main direction.
...
(defn mcu-model
  ...
  side of the PCB facing ônorthö and centering at the origin of the local
  cordinate system. The connector itself is placed on the ôeastö (positive x)
  side."
...
(defn mcu-lock-bolt
  "Parts of the lock-style MCU support that donÆt integrate with the case.
...
(defn connection-position [getopt shape]
  (let [use-housing (and (getopt :case :rear-housing :include)
   ...
   (->> shape
        ...
        (translate alignment)
        ;; Rotate to face the cornerÆs main direction.
        (maybe/rotate [0 0 (- (matrix/compass-radians (first corner)))])
...
(defn connection-socket
  ...
  based on the assumption that the socket is pointing ônorthö and the wires
  come out to the ôsouthö. The hole is slightly thicker than the wall for
  cleaner rendering."
  [getopt]
  (let [socket-size (getopt :connection :socket-size)
        thickness (getopt :case :web-thickness)]
