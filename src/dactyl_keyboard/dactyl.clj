...
(def α (/ π 20))                        ; curvature of the columns
(def β (/ π 20))                        ; curvature of the rows
(def centerrow (- nrows 3))             ; controls front-back tilt
(def centercol 0.7)                       ; controls left-right tilt / tenting (higher number is more tenting)
(def tenting-angle (/ π 5))            ; or, change this for more precise tenting control
(def column-style
  (if (> nrows 5) :standard :standard))  ; options include :standard, :orthographic, and :fixed
...
(def teensy-inset -1.6)                 ; if teensy pokes out edge or is not connected. negative moves outside
  ...
  (apply union
         (for [column columns
               ...
               :when (or (.contains [2 3] column)
                         (not= row lastrow))]
           (->> (if (= column 6) single-plate single-plate )
...
(def right-wall
  (let [tr (if (true? pinky-15u) web-post-tr web-post-tr)
        br (if (true? pinky-15u) web-post-br web-post-br)]
...
(def case-walls
  (union
       ...
       (for [x (range 4 ncols)] (key-wall-brace x cornerrow 0 -1 web-post-bl x       cornerrow 0 -1 wide-post-br))
       (for [x (range 5 ncols)] (key-wall-brace x cornerrow 0 -1 web-post-bl (dec x) cornerrow 0 -1 wide-post-br))
       ; thumb walls
...
(def teensy-pcb-thickness 2.5)
(def teensy-holder-width  (+ 5 teensy-pcb-thickness))
(def teensy-holder-height (+ 6 teensy-width))
(def teensy-offset-height 5)
(def teensy-holder-top-length 18)
(def teensy-top-xy (key-position 0 (- centerrow 1) (wall-locate3 teensy-inset 0)))
(def teensy-bot-xy (key-position 0 (+ centerrow 1) (wall-locate3 teensy-inset 0)))
...
(def pinky-walls
  (union
    (key-wall-brace lastcol cornerrow 0 -1 web-post-br lastcol cornerrow 0 -1 web-post-br)
